...
#define do_func(a, b, c, d, e, f, g, h) (2 + 4 * (a) - 3 * (b) + 5 * (c) + 6 * (d) - 2 * (e) + 7 * (f) - 8 * (g) + 9 * (h))

const float max = do_func(100, 100, 100, 100, 100, 100, 100, 100);

/* float do_func(float a, float b, float c, float d, float e, float f, float g, float h) { */
/*   return 2 + 4 * a - 3 * b + 5 * c + 6 * d - 2 *e + 7 * f - 8 * g + 9 * h; */
/* } */

float mul_max(float x) {
  return (x * 2 *  max) - max;
}
  ...
  for (size_t i = 0; i < num_train; ++i) {
	for (size_t j = 0; j < num_var; ++j) {
	  ...
	  if (j == num_var - 1) {
		...
		g = train[i * num_var + 6];
		h = train[i * num_var + 7];
		train[i * num_var + num_var - 1] = (do_func(a,b,c,d,e,f,g,h) + max ) / (max * 2);
	  }
	}
  }
  for (size_t i = 0; i < num_train; ++i) {
	for (size_t j = 0; j < num_var; ++j) {
	  ...
	  if (j == num_var - 1) {
		...
		g = val[i * num_var + 6];
		h = val[i * num_var + 7];
		val[i * num_var + num_var - 1] = (do_func(a,b,c,d,e,f,g,h) + max ) / (max * 2);
	  }
	}
  }
