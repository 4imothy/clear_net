...
int main(void) {
	...
	Matrix train_input = mat_form(num_train_files, num_pixels, train.ncols, &MAT_GET(train, 0, 0));
    Matrix train_output = mat_form(num_train_files, 1, train.ncols, &MAT_GET(train, 0, num_pixels));
	...
	Matrix test_input = mat_form(num_test_files, num_pixels, test.ncols, &MAT_GET(test, 0, 0));
    Matrix test_output = mat_form(num_test_files, 1, test.ncols, &MAT_GET(test, 0, num_pixels));
    ...
    for (size_t i = 0; i < num_epochs; ++i) {
      for (size_t batch_num = 0; batch_num < (num_train_files / batch_size);
           ++batch_num) {
        net_get_batch(&batch_input, &batch_output, train_input, train_output,
