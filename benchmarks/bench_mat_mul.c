...
void better_cache_use(Matrix dest, Matrix left, Matrix right, size_t pointless) {
    CLEAR_NET_ASSERT(left.ncols == right.nrows);
    size_t n = left.ncols;
    CLEAR_NET_ASSERT(dest.nrows == left.nrows);
    CLEAR_NET_ASSERT(dest.ncols == right.ncols);
...
void bench(Matrix dest, Matrix left, Matrix right, MulFunc func, char *name) {
    ...
    printf("on average the %s solution took: %f seconds\n",
        	name, total_time / (double)NUM_ITERS);
}
...
void strassen(Matrix dest, Matrix srcA, Matrix srcB, size_t length)
{
    ...
    Matrix a11 = alloc_mat(len, len), a12 = alloc_mat(len, len), a21 = alloc_mat(len, len), a22 = alloc_mat(len, len),
                           b11 = alloc_mat(len, len), b12 = alloc_mat(len, len), b21 = alloc_mat(len, len), b22 = alloc_mat(len, len), c11 = alloc_mat(len, len), c12 = alloc_mat(len, len), c21 = alloc_mat(len, len), c22 = alloc_mat(len, len), m1 = alloc_mat(len, len), m2 = alloc_mat(len, len), m3 = alloc_mat(len, len), m4 = alloc_mat(len, len), m5 = alloc_mat(len, len), m6 = alloc_mat(len, len), m7 = alloc_mat(len, len), temp1 = alloc_mat(len, len), temp2 = alloc_mat(len, len);
