...
void do_test_with_default_elements(Padding padding, Activation act, const size_t input_rows, const size_t input_cols, size_t krows, size_t kcols) {
    ...
    cn_forward_convolutional(&cl, &minput, 1);
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "same_zeros") == 0) {
        const size_t dim = 15;
        ConvolutionalLayer cl = cn_init_convolutional_layer(Same, default_act, 15, 15, 3, 3);
        cn_alloc_filter(&cl, 1);
        ...
        const size_t dim = 10;
        ConvolutionalLayer cl = cn_init_convolutional_layer(Same, default_act, 10, 10, 3, 3);
        cn_alloc_filter(&cl, 1);
        ...
        const size_t dim = 20;
        ConvolutionalLayer cl = cn_init_convolutional_layer(Same, default_act, dim, dim, 3, 3);
        cn_alloc_filter(&cl, 1);
        ...
        const size_t dim = 20;
        ConvolutionalLayer cl = cn_init_convolutional_layer(Same, default_act, dim, dim, 5, 5);
        cn_alloc_filter(&cl, 1);
        ...
        cls[0] = cn_init_convolutional_layer(Valid, default_act, in_dim, in_dim, 5,5);
        cn_alloc_filter(&cls[0], 3);
        cn_alloc_filter(&cls[0], 3);
        cn_alloc_filter(&cls[0], 3);
        _cn_randomize_convolutional_layer(&cls[0], -1, 1);

        cls[1] = cn_init_convolutional_layer(Valid, default_act, cls[0].output_nrows, cls[0].output_ncols, 5, 5);
        cn_alloc_filter(&cls[1], 2);
        cn_alloc_filter(&cls[1], 2);
        _cn_randomize_convolutional_layer(&cls[1], -1, 1);

        cls[2] = cn_init_convolutional_layer(Valid, default_act, cls[1].output_nrows, cls[1].output_ncols, 7, 7);
        ...
        cn_forward_convolutional(&cls[0], &minput, 1);
        cn_forward_convolutional(&cls[1], cls[0].outputs, cls[0].nfilters);
        cn_forward_convolutional(&cls[2], cls[1].outputs, cls[1].nfilters);
        printf("zero output\n");
        ...
        size_t in_dim = 28;
        ConvolutionalLayer *cls = CLEAR_NET_ALLOC(n_cls * sizeof(ConvolutionalLayer));
        cls[0] = cn_init_convolutional_layer(Valid, default_act, in_dim, in_dim, 8, 8);
        cn_alloc_filter(&cls[0], 3);
        cn_alloc_filter(&cls[0], 3);
        cn_alloc_filter(&cls[0], 3);
        cn_alloc_filter(&cls[0], 3);
