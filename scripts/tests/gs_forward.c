...
int main(int argc, char *argv[]) {
    ...
    if (strequal(argv[1], "dense")) {
        ...
        for (size_t i = 0; i < dout; ++i) {
            CLEAR_NET_ASSERT(GET_NODE(VEC_AT(gsout, i)).num - VEC_AT(fout, i) <
                             eps);
        }
        printf("Pass: Dense Forward\n");
    } else if (strequal(argv[1], "conv")) {
        size_t in_width = 20;
        size_t in_height = 20;
        ...
        for (size_t i = 0; i < fes.nrows; ++i) {
            for (size_t j = 0; j < fes.ncols; ++j) {
                CLEAR_NET_ASSERT(
                    MAT_AT(fes, i, j) - GET_NODE(MAT_AT(gses, i, j)).num < eps);
            }
        }
        printf("Pass: convolutional\n");
    } else if (strequal(argv[1], "with_pooling")) {
        size_t in_width = 28;
        size_t in_height = 28;
        Net net = cn_alloc_conv_net(in_height, in_width, 1);
        cn_alloc_conv_layer(&net, Valid, Sigmoid, 3, 9, 9);
        ...
        for (size_t i = 0; i < fes.nelem; ++i) {
            CLEAR_NET_ASSERT(VEC_AT(fes, i) - GET_NODE(VEC_AT(gses, i)).num <
                             eps);
        }
