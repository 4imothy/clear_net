...
float poss_kernel_elements[] = {2, 6, 7, 8, 4, 0, 6, 4, 2, 0, 9, 7, 5, 9, 8, 8,
...
void do_test(Padding padding, Activation act, const ulong input_nrows,
             const ulong input_ncols, ulong kern_nrows, ulong kern_ncols,
             scalar *kern_pool, ulong kern_pool_len, scalar *input_pool,
             ulong input_pool_len) {
...
int main(int argc, char *argv[]) {
    ...
    if (strequal(argv[1], "same_zeros")) {
        ...
        do_test(Same, default_act, dim, dim, 3, 3, elem, elem_len,
                poss_elements, poss_elements_len);
    }
        ...
        ulong elem_len = LEN(elem);
        do_test(Same, default_act, dim, dim, 5, 5, elem, elem_len,
                poss_elements, poss_elements_len);
    }

    else if (strequal(argv[1], "same_even_kernel")) {
        do_test(Same, default_act, 20, 20, 4, 4, poss_kernel_elements,
                poss_kernel_elem_len, poss_elements, poss_elements_len);
    }

    else if (strequal(argv[1], "same_rect")) {
        do_test(Same, default_act, 30, 30, 5, 3, poss_kernel_elements,
                poss_kernel_elem_len, poss_elements, poss_elements_len);
    }

    else if (strequal(argv[1], "full_7x7")) {
        do_test(Full, default_act, 30, 30, 7, 7, poss_kernel_elements,
                poss_kernel_elem_len, poss_elements, poss_elements_len);
    }

    else if (strequal(argv[1], "full_even")) {
        do_test(Full, default_act, 15, 15, 4, 4, poss_kernel_elements,
                poss_kernel_elem_len, poss_elements, poss_elements_len);
    }

    else if (strequal(argv[1], "full_rect")) {
        do_test(Full, default_act, 30, 30, 4, 7, poss_kernel_elements,
                poss_kernel_elem_len, poss_elements, poss_elements_len);
    }

    else if (strequal(argv[1], "valid_7x7")) {
        do_test(Valid, default_act, 11, 11, 7, 7, poss_kernel_elements,
                poss_kernel_elem_len, poss_elements, poss_elements_len);
    }

    else if (strequal(argv[1], "valid_rect")) {
        do_test(Valid, default_act, 23, 23, 1, 6, poss_kernel_elements,
                poss_kernel_elem_len, poss_elements, poss_elements_len);
    }

    else if (strequal(argv[1], "valid_rect_input")) {
        do_test(Valid, default_act, 10, 20, 4, 4, poss_kernel_elements,
                poss_kernel_elem_len, poss_elements, poss_elements_len);
    }
}
