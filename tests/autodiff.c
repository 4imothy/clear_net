...
#define PRINT_VAL(x)                                                           \
    printf("%s %f %f\n", #x, getVal((cg), (x)), getGrad((cg), (x)))

int main(int argc, char *argv[]) {
    scalar leaker = 0.1;
    CLEAR_NET_ASSERT(argc == 2);
    CompGraph *cg = allocCompGraph(0);
    if (strequal(argv[1], "1")) {
        ...
        ulong c = elu(cg, b, leaker);
        ulong d = mul(cg, a, sub(cg, c, b));
        ulong e = mul(cg, d, d);
        ulong f = elu(cg, e, leaker);
        backward(cg, f, leaker);
        PRINT_VAL(a);
        PRINT_VAL(b);
        PRINT_VAL(c);
